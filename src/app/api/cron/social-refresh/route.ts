...
import { refreshInstagramConnections } from '@/actions/social';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const profile of profiles.documents) {
      ...
      if (!result.success) {
        failures.push(`${chatId}: ${result.error || 'refresh failed'}`);
        continue;
      }

      refreshedConnections += Number('refreshed' in result ? result.refreshed || 0 : 0);

      if ('errors' in result && Array.isArray(result.errors) && result.errors.length > 0) {
        failures.push(...result.errors.slice(0, 3).map((error) => `${chatId}: ${error}`));
      }
    }
    ...
    const errorMessage = error instanceof Error ? error.message : 'Social refresh cron failed';
